...
SETTINGS1 = {
    ...
    "romfiles" : ["Final\material\8700340.rom","Final\material\8700339.rom"],
...
print_statements.append(six.text_type(TRACKER.get_tool_descriptions()))
...
def save_pcd_file(file_path, coordinates):
    ...
    print("The output is saved as a PCD file.")
...
def realtime_point():
    ...
    print_statements.append(six.text_type(frame_data))
    t = frame_data

    # Extract q and t from NDI
    Data_point_tool_q = (t[3][0][0])
    Data_point_ref_q = (t[3][1][0])

    # Check if any quaternion or translation vector component is NaN
    if np.isnan(Data_point_tool_q).any() or np.isnan(Data_point_ref_q).any():
...
def on_release(key):
    ...
    if key == keyboard.Key.space:
        ...
        save_pcd_file("Final\material\gather_point.pcd", coordinates)
