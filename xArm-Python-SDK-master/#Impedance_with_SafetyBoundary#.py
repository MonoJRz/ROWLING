#IMPEDANCEC CONTROL WITH SAFETY BOUNDARY #
import os
import sys
import time
import math
...
ref_frame = 0   
# Boundary limits
x_max, x_min, y_max, y_min, z_max, z_min = [600, 100,800, -800, 800, -500]
try:
            ...
            if code == 0:
                ...
                if motion_mode_activated:
                    ...
                    while x1 > x_max:
                            ...
                            print("New position:", new_position)
                            print("New orientation:", (new_roll_deg, new_pitch_deg, new_yaw_deg))
                            #print("Robot position exceeds boundary x limits. returning to boundary.")
                            x1, y1, z1 = new_position
                            # Adjust the force in X axis
                            force_x1 = 2000 * ((x1 - x_max)/1000)
                            # Ensure the force is non-zero
                            print(force_x1)
                            force_ref = [-force_x1, 0, 0.0, 0, 0, 0]
                    ...
                    while  x1 < x_min:
                            ...
                            x1, y1, z1 = new_position
                            # Adjust the force in X axis
                            force_x2 = 2000 * ((x_min - x1)/1000) # min 300 
                            # Ensure the force is non-zero
                            print(force_x2)
                            force_ref = [force_x2, 0, 0.0, 0, 0, 0]
                            force_axis = [1, 0, 0, 0, 0, 0]
                            arm.config_force_control(ref_frame,  force_axis, force_ref, [0]*6)
                            # enable ft sensor communication
                            arm.ft_sensor_enable(1)
                    ...
                    while  y1 > y_max:
                            ...
                            print("New orientation:", (new_roll_deg, new_pitch_deg, new_yaw_deg))
                            x1, y1, z1 = new_position
                            # Adjust the force in y axis
                            force_y1 = 2000 * ((y1 - y_max)/1000)
                            print(force_y1)
                            # Ensure the force is non-zero
                            force_ref = [0, force_y1, 0.0, 0, 0, 0]
                            force_axis = [0, 1, 0, 0, 0, 0]
                            arm.config_force_control(ref_frame,  force_axis, force_ref, [0]*6)
                            # enable ft sensor communication
                            arm.ft_sensor_enable(1)
                            arm.ft_sensor_app_set(2)
                    ...
                    while  y1 < y_min:
                            ...
                            x1, y1, z1 = new_position
                            # Adjust the force in y axis
                            force_y2 = 2000 * ((y_min - y1)/1000)
                            time.sleep(0.01)
                            # Ensure the force is non-zero
                            force_ref = [0, force_y2, 0.0, 0, 0, 0]
                            force_axis = [0, 1, 0, 0, 0, 0]
                            arm.config_force_control(ref_frame,  force_axis, force_ref, [0]*6)
                            # enable ft sensor communication
                            arm.ft_sensor_enable(1)
                    ...
                    while z1 > z_max:
                            ...
                            x1, y1, z1 = new_position
                            # Adjust the force in X axis
                            force_z1 = 2000 * ((z1 - z_max)/1000)
                            print(force_z1)
                            force_ref = [0, 0, -force_z1, 0, 0, 0]
                    ...
                    while  z1 < z_min:
                            ...
                            x1, y1, z1 = new_position
                            # Adjust the force in X axis
                            force_z2 = 2000 * ((z_min + z1)/1000)
                            print(force_z2)
                            force_ref = [0, 0, force_z2, 0, 0, 0]
                            force_axis = [0, 0, 1, 0, 0, 0]
                            arm.config_force_control(ref_frame,  force_axis, force_ref, [0]*6)
                            # enable ft sensor communication
                            arm.ft_sensor_enable(1)
                    ...
                    if x1 < x_max and x1 > x_min and y1 < y_max and y1 > y_min and z1 < z_max and z1 > z_min:
                            arm.clean_warn()
                            arm.clean_error()
                            arm.motion_enable(enable=True)
